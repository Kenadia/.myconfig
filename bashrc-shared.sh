...
alias glow='git log --oneline --format="%h %<(50,trunc)%s" | less -R'
function glo () {
  N=$1
  glow | head -n ${N:=5}
}
...
alias gpuf='git push -f -u origin $(git rev-parse --abbrev-ref HEAD)'
  # rebase
alias gr='git rebase'
